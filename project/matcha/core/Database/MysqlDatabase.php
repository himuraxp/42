...
class MysqlDatabase extends Database{
	...
	private function getPDO(){
		if($this->pdo === null){
			$pdo = new PDO('mysql:dbname=match;host=localhost:8889', 'root', 'root');
	...
	public function drop($database = null, $table = null){
		if (is_null($table)){
			$req = $this->getPDO()->exec("DROP " . $database . " IF EXISTS ". $this->db_name);
			return echo $this->db_name . " has was deleted !" . PHP_EOL;
	...
	public function create($db_table){
		$req = $this->getPDO()->exec("CREATE DATABASE IF NOT EXISTS ". $this->db_name);
		if ($req){
			echo "Database : ". $this->db_name . " created" . PHP_EOL;
		} else {
		...
		$req = $this->getPDO()->exec("
			...
			");
		echo " Table: " . $db_table['users'] . " created" . PHP_EOL;
		...
		$req = $this->getPDO()->exec("
			...
			link TEXT NOT NULL);
			");
		echo " Table: " . $db_table['pictures'] . " created" . PHP_EOL;
		...
		$req = $this->getPDO()->exec("
			...
			");
		echo " Table: " . $db_table['messages'] . " created" . PHP_EOL;
		...
		$req = $this->getPDO()->exec("
			...
			");
		echo " Table: " . $db_table['likes'] . " created" . PHP_EOL;
		...
		$req = $this->getPDO()->exec("
			...
			");
		echo " Table: " . $db_table['styles'] . " created" . PHP_EOL;
	}
