  ...
  function  xD(popId) {
    ...
    if(!(document.getElementById("pop_"+popId))) {
      ...
      $.getScript( "/js/actionPopup.js", function( data, textStatus, jqxhr ) {
        ...
        io.socket.post('/movie/download', {magnet_link: movie2, eventpop: popId, loadpop: loadPop},function(resData, jwres){
        });
        io.socket.on('stream_'+popId, function(data){
          ...
          $("#pop_"+popId).children(".player").html('<video style="margin-bottom:-4px;" width="100%" height="auto" controls>'+
